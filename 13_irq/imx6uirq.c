...
static int keyio_init(void)
{
	...
	for (i = 0; i < KEY_NUM; i++) {
		...
		gpio_direction_input(imx6uirq.irqkeydesc[i].gpio);		/* 将 GPIO 引脚设置为输入模式 */
		imx6uirq.irqkeydesc[i].irqnum = irq_of_parse_and_map(imx6uirq.nd, i);	/* 解析设备树节点中的 interrupts 属性，映射中断号 */
#if 0
