...
static struct i2c_driver ft5x06_ts_driver = {
	.driver = {
		.owner = THIS_MODULE,		/* 声明模块所有者 */
		.name = "edt_ft5x06",		/* 驱动名称，出现在/sys/bus/i2c/drivers/目录下 */
		.of_match_table = of_match_ptr(ft5x06_of_match),		/* 指向设备树匹配表 */
	},
	.id_table = ft5x06_ts_id,		/* 指向传统匹配表 */
	.probe    = ft5x06_ts_probe,	/* 设备匹配成功后调用的初始化函数 */
	.remove   = ft5x06_ts_remove,	/* 设备移除或驱动卸载时调用的清理函数 */
};
...
{
	int ret = 0;
	ret = i2c_add_driver(&ft5x06_ts_driver);	/* 向内核注册i2c驱动 */
	return ret;
}
...
{
	i2c_del_driver(&ft5x06_ts_driver);			/* 向内核注销i2c驱动 */
}

module_init(ft5x06_init);			/* 模块入口 */
module_exit(ft5x06_exit);			/* 模块出口 */
MODULE_LICENSE("GPL");				/* 声明模块许可类型 */
MODULE_AUTHOR("zuozhongkai");		/* 声明模块作者信息 */
