...
struct timer_dev timerdev;		/* 表示一个具体的定时器设备实例 */
...
static int led_init(void)
{
	...
	/* 初始化led所使用的IO */
	ret = gpio_request(timerdev.led_gpio, "led");		/* 请求IO 	*/
	if (ret) {
...
static int __init timer_init(void)
{
	...
	if (timerdev.major) {
		...
		register_chrdev_region(timerdev.devid, TIMER_CNT, TIMER_NAME);	/* 注册设备号 */
	} else {
	...
	cdev_init(&timerdev.cdev, &timer_fops);		/* 关联文件操作集 */
	/* 3、添加字符设备到内核 */
